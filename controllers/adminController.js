...
exports.createRallyControllers = async(req, res) =>{
    let {mainTitle, subTitle, content, time, address, bulletin, mapUrl} = req.body
    ...
    try {
        ...
        db.query(setUpStatusSql, (err, results) =>{
            ...
            const createRallySql = 'INSERT INTO rally (mainTitle, subTitle, content, time, address, bulletin, image, mapUrl) VALUE(?, ?, ?, ?, ?, ?, ?, ?)'
            db.query(createRallySql, [mainTitle, subTitle, content, time, address, bulletin, imageUrl, mapUrl], (err, results) =>{
                if (err) {
                    return res.send({code: 1, message: err.message});
                };
